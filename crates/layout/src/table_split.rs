...
//! - [`split_row_in_cells`] — the continuation of ONE row taller than a
//!   whole page: every cell's content is cut at the deepest line boundary
...
//!   hit-testing still maps to the model (`content_offset`).
...
/// Issue #91 — continue a row taller than `avail` inside its cells. Each
...
/// Returns `None` when no cell could place a single line — the caller
/// then falls back to atomic placement (`OversizeLine`). Tail cells are
/// top-aligned (a `vAlign` offset does not survive the cut).
pub(crate) fn split_row_in_cells(row: &TableRowBox, avail: f32) -> Option<RowSplit> {
    ...
    for cell in &row.cells {
        ...
        let (head, tail, tail_first) = if inner > 0.0 {
            split_cell_blocks(&cell.content, inner)
        } else {
            (Vec::new(), cell.content.clone(), 0)
        };
        if !head.is_empty() {
            progressed = true;
