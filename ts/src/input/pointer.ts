...
/**
 * Wire pointer listeners on `canvas`. Returns a teardown that removes them.
 * Pointer events still fire on a `<canvas>` whose drawing surface has been
 * transferred to the worker, so this is safe to attach post-transfer.
 */
export function attachPointer(canvas: HTMLCanvasElement, client: EngineClient): () => void {
    let dragging = false;
